import React from 'react';

import type { IconProps } from '@/interfaces/IconProps.interface.tsx';

export const FacebookIconComponent: React.FC<IconProps> = ({ className, title }) => (
    <svg width="35" height="35" viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
        <title>{title}</title>
        <g clipPath="url(#clip0_1_7061)">
            <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M17.4443 0.530273C20.5993 0.566276 23.4496 1.34168 25.995 2.8565C28.5102 4.34106 30.6024 6.44606 32.0716 8.97019C33.5772 11.531 34.3479 14.3985 34.3838 17.5727C34.2944 21.9158 32.9245 25.6253 30.2742 28.701C27.6239 31.7768 24.2296 33.6796 20.7157 34.4091V22.2324H24.0378L24.789 17.4472H19.7587V14.3131C19.7308 13.6634 19.9362 13.0251 20.338 12.5137C20.7403 12.0008 21.4487 11.7313 22.4632 11.7051H25.5009V7.51337C25.4573 7.49934 25.0437 7.44389 24.2602 7.34701C23.3715 7.24304 22.4779 7.1875 21.5832 7.18065C19.5583 7.18999 17.9568 7.76119 16.7788 8.89424C15.6008 10.027 14.9991 11.6658 14.9736 13.8107V17.4472H11.1454V22.2324H14.9736V34.4091C10.6591 33.6796 7.26468 31.7768 4.61439 28.701C1.9641 25.6252 0.594311 21.9158 0.504883 17.5727C0.540662 14.3984 1.31138 11.5308 2.81704 8.97019C4.28627 6.44606 6.37849 4.34107 8.89362 2.8565C11.439 1.34198 14.2893 0.566568 17.4443 0.530273Z"
                fill="#EF4934"
            />
        </g>
        <defs>
            <clipPath id="clip0_1_7061">
                <rect width="35" height="35" fill="white" />
            </clipPath>
        </defs>
    </svg>
);
